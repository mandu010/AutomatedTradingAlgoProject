...
class WilliamsIndicators(object):
    ...
    def check_bull_bar_dataframe(self, candles_2rows_dataframe):
        ...
        if float(candles_2rows_dataframe['intl'][0]) > float(candles_2rows_dataframe['intl'][1]) and \
                float(candles_2rows_dataframe['intc'][1]) > \
                (float(candles_2rows_dataframe['inth'][1]) + float(candles_2rows_dataframe['intl'][1])) / 2:
                    return True
        else:
            return False
    ...
    def check_bull_bar(self, candles_2rows_list):
        ...
        :return True if current bar is bull divergent bar, else False
        :param candles_2rows_dataframe: dataframe that consists of two rows of candles in chronological order, created by get_num_rows(self, n)
        candles indicies: 0 - Open time, 1 - Open, 2 - inth, 3 - intl, 4 - intc, 5 - Volume, 6 - intc time
        '''
    ...
    def check_bear_bar_dataframe(self, candles_2rows_dataframe):
        ...
        if float(candles_2rows_dataframe['inth'][0]) < float(candles_2rows_dataframe['inth'][1]) and \
                float(candles_2rows_dataframe['intc'][1]) < \
                (float(candles_2rows_dataframe['inth'][1]) + float(candles_2rows_dataframe['intl'][1])) / 2:
            return True
        else:
    ...
    def SMMA(self, candles_list, n_smoothing_periods, future_shift):
        ...
        for i in range(len(candles_list)):
            median_price = (float(candles_list[i][2]) + float(candles_list[i][3])) / 2  # (inth + intl) / 2
    ...
    def check_bar_type(self, two_candles_list):
        ...
        candles indicies: 0 - Open time, 1 - Open, 2 - inth, 3 - intl, 4 - intc, 5 - Volume, 6 - intc time
        Logic: first number - 1 if open is in upper 1/3 of inth-intl, range, 3 - in intler
        '''
